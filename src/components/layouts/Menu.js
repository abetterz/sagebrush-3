                ...
                <ul className="main-menu">
                    {navigationmenu.length > 0 ? navigationmenu.map((item, i) => (
                        <li key={i} className={`menu-item ${item.child ? 'menu-item-has-children' : ''} `} onClick={this.triggerChild}>
                            {item.child ? <Link onClick={e => e.preventDefault()} to="/" className="text-light-white"> {item.linkText} </Link> : (item.link.startsWith('http') ? <a href={item.link} target="_blank" className="text-light-white">{item.linkText}</a> : <Link to={item.link} className="text-light-white"> {item.linkText} </Link>)}
                            {item.child ?
